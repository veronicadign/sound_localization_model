"""
Directly lifted from brian2hears.
I needed it to create the database indexed in deg rather than rad,
and found this to be the best option
"""

import numpy as np
from brian2 import metre, ms, second, usecond
from brian2hears import HRTFDatabase
from brian2hears.filtering.fractionaldelay import FractionalDelay
from brian2hears.hrtf import *
...
class HeadlessDatabase(HRTFDatabase):
    ...
    def __init__(
        ...
        if itd is None:
            ...
            itd = diameter * np.sin(np.deg2rad(azim)) / speed_of_sound_in_air
