...
positions and model->head rotations.

Every ABR entry point (main_abr*.py) and every figure script imports from here.
Before this module the same constants were copied into five files and had already
drifted apart; nothing outside this module should declare them again.
...
PROVENANCE OF THE NUCLEUS POSITIONS.  All five are now derived from human
atlases (ABR_reconstruction/atlas/, RESULTS/atlas_validation/):
    ...
    MNTB  no atlas measures it (ANCHOR annotates the trapezoid BODY, not the
          NUCLEUS), so the Kulesza 2015 morphometry is kept and re-anchored onto
          the atlas-derived SOC position

They are stored as MNI coordinates and converted to the head frame at import, so
editing an atlas value or HEAD_CENTRE_MNI_MM propagates to the whole pipeline.
The positions they replaced are listed below and in test_head_geometry.py.

ATLAS_CANDIDATES still loads the full comparison table (every candidate for every
generator, both head-frame origins) for reporting; it is not what the pipeline
reads.
"""
...
# The atlas derivation and the LSO axon builder live outside this package; they are
# imported lazily (below) and need their own directories on sys.path.
...
# requires r < r_scalp), so they sit 1 um under it.  Derived from the radii, so
# changing FOUR_SPHERE_RADII can no longer silently invalidate it.
_ELECTRODE_INSET_UM = 1.0
SCALP_R = FOUR_SPHERE_RADII[3] - _ELECTRODE_INSET_UM       # 89_999. um

# Backwards-compatible alias (main_abr.py exported this name).
_SCALP_R = SCALP_R
...
# volume conductor.
#
# A1/A2 are the earlobe labels for the same two sites and were used by the figure
# scripts while the simulation used M1/M2; both names are provided here so the
# two halves of the repo stop disagreeing.  T7/T8 are extra lateral sites used
# only by plots/electrodes.py.
...
# Nucleus positions
# ===========================================================================
# NOT hardcoded in head-frame micrometres any more.  The primary quantity is the
# ATLAS-MEASURED MNI152 coordinate of each generator; the head-frame position the
# ABR pipeline consumes is DERIVED from it here, so changing an atlas value or
# the head-frame origin propagates everywhere automatically.
#
...
#   MNTB       No atlas answer exists - ANCHOR annotates the trapezoid BODY but
#              not the trapezoid NUCLEUS.  Derived below from the Sitek SOC
#              anchor plus the Kulesza (2015) morphometry the model already used.
#
# Previous values, replaced by the above (head frame, um, right side; left
# mirrored in x).  None of them was atlas-derived; the MSO's had no recorded
# derivation at all:
#   MSO  [ 5000, -18700, -29520]      LSO  [ 9000, -20500, -30000]
#   GBC  [10000, -38000, -35000]      SBC  [ 9400, -36500, -35000]
#   MNTB [  400, -16700, -29520]
# ---------------------------------------------------------------------------

#: Origin of the head frame, in MNI mm.  Derived by least-squares fitting the
#: model's 79 mm brain shell to the MNI152 brain-mask surface; regenerate with
...
def mni_mm_to_head_um(p_mni_mm, centre=None):
    """MNI152 mm -> head-centred micrometres (the pipeline's native unit)."""
    c = HEAD_CENTRE_MNI_MM if centre is None else np.asarray(centre, float)
    return (np.asarray(p_mni_mm, dtype=float) - c) * 1e3


# ---------------------------------------------------------------------------
# Brainstem anatomical axes expressed in MNI, measured (not assumed) from the
# route-A SOC -> IC vector.  Used to apply the MNTB's literature offset along
# the true neuraxis rather than along a head axis.
# ---------------------------------------------------------------------------
...
# cross-modality spread.  Everything downstream (the mirrored ROTATION matrices,
# the figures, the L/R checks) assumes mirror symmetry, so the values actually
# used are the symmetrised ones below; the raw pair is kept here so the
# asymmetry is visible rather than silently discarded.
# ---------------------------------------------------------------------------
ATLAS_MNI_MM_RAW = {
...
# is kept and RE-ANCHORED onto the new SOC position -
#   ~400 um from the midline, ~2 mm rostral to the SOC centroid.
# The rostral step is taken along the measured neuraxis (BRAINSTEM_ROSTRAL_MNI),
# not along a head axis, which is what "rostral" anatomically means.
...
# The SBC field's displacement from the GBC field.  These used to GENERATE
# SBC_POS_UM; now both fields are measured separately and these are diagnostics
# (still exported because main_abr_avcn.py imports the names).
_SBC_DELTA_UM = SBC_POS_UM['R'] - AVCN_POS_UM['R']
SBC_ROSTRAL_OFFSET_UM = np.array([0., _SBC_DELTA_UM[1], _SBC_DELTA_UM[2]])
SBC_MEDIAL_OFFSET_UM = float(-_SBC_DELTA_UM[0])       # + = toward the midline
...
# ---------------------------------------------------------------------------
# MSO: model axes -> head axes
#
# MSO HOC (from LAYER_BOUNDARIES): medial dend z>0, lateral dend z<0.
#
# head_x: mediolateral
#   Right MSO: medial dend (+model_z) -> toward midline -> -head_x
#   Left  MSO: medial dend (+model_z) -> toward midline -> +head_x
# head_y: anteroposterior (tonotopic: low CF posterior, high CF anterior).
#   model_x increases = higher CF = more anterior -> head_y = +model_x, same sign
#   both sides (tonotopy runs the same anatomical direction bilaterally).
...
# (AXON_TARGET = [-1,0,-1]) - points at the CONTRALATERAL MNTB, i.e. crosses the
# midline on BOTH sides.  Follows the MSO convention: flip head_x and head_z
...
#   R -> [-1,+1,0]  head_x=-1  toward the midline from the RIGHT AVCN  -> crosses
#   L -> [+1,+1,0]  head_x=+1  toward the midline from the LEFT  AVCN  -> crosses
#
...
# -/+AVCN_ROSTRAL_TILT_DEG swings the rostral end (+head_y) toward +head_x on the
# RIGHT and -head_x on the LEFT.  The midline crossing survives any tilt; set the
# angle to 0 (or --avcn-tilt-deg 0) to recover the untilted mapping.
# ---------------------------------------------------------------------------
...
def build_avcn_rotation(tilt_deg):
    """Side-specific AVCN model->head rotation with an outward rostral tilt."""
...
#             generator; giant mediolateral axial current)
#   model_y = principal-cell axon (efferent to MSO/LSO/SPN) -> rostrocaudal
#   model_z = principal dendritic long axis (bipolar tufts)
#
# Base map:
#   RIGHT: identity -> calyx model_x -> +head_x, dendrite model_z -> head_z.
#   LEFT : sagittal mirror diag(-1,1,-1) -> calyx -> -head_x.
# So the calyx crossing dipole is mediolateral and MIRRORED across sides, sums
# COHERENTLY bilaterally, and is consistent with the AVCN GBC axon orientation.
#
# The MNTB principal long (dendritic) axis is ~73+-5 deg (transverse) / 79+-7 deg
# (coronal) from horizontal, dendrites PERPENDICULAR to the mediolateral
# trapezoid-body fibres, and the whole SOC has a slight superior-rostral tilt.
# All three are realised by a single tilt about head_x, which leaves the dominant
...
# frame; build_lso_axon.py) and the ascending lateral lemniscus climbs toward the
# IC = +head_z.  Both sides map AXON_DIR -> +head_z (axons ascend bilaterally).
# The mediolateral axis is mirrored per side so "medial" (+model_z) always points
# at the midline:
#   Right LSO (at +head_x): medial -> -head_x   (s = -1)
#   Left  LSO (at -head_x): medial -> +head_x   (s = +1)
# Consequence: head_z (axon->IC) is IDENTICAL on both sides while head_x and
# head_y mirror.  This differs from the MSO (which mirrors head_x + head_z and
# keeps head_y) because here head_z is the axon axis and must not flip.  It keeps
# tonotopy (mediolateral) anatomically consistent bilaterally (lateral = low CF,
# medial = high CF).
# ---------------------------------------------------------------------------
def lso_rotation(a, side):
    """Proper rotation (det=+1): R @ a = +head_z; R @ model_z = -/+head_x for R/L.

    ey = ez x ex with orthonormal {ex, ez} is right-handed -> det = +1 by
    construction.
    """
    ...
    if np.linalg.norm(ex) < 1e-6:                         # a parallel to z fallback
...
def mni_to_head(p_mni_mm, source='fitted'):
    """MNI152 mm -> head-centred mm.  See atlas/mni_head_transform.py."""
...
def head_to_mni(p_head_mm, source='fitted'):
    """Head-centred mm -> MNI152 mm.  See atlas/mni_head_transform.py."""
    sys.path.insert(0, _ATLAS_DIR)
...
def load_atlas_candidates():
    ...
    pipeline - reported only, so that adopting a candidate set is a deliberate,
